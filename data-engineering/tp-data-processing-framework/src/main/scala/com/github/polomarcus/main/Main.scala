...
object Main {
  def main(args: Array[String]) {
    ...
    // Tips: use a groupBy
    val climateChangeClimateNews=enrichedDataset.groupBY($"containsWordGlobalWarming").count()
    climateChangeClimateNews.show()
    ...
    val sqlDF = spark.sql("SELECT * FROM news")
    sqlDF.show()
    ...
    filteredNewsAboutClimate.select("title").show()
    ...
    filteredNewsAboutClimate.write.partitionBy("date","media").parquet("parquet")
